...
function App() {
  ...
  useEffect(() => {
    // get params "model" from url
    const urlParams = new URLSearchParams(window.location.search);
    const modelUrlEnCoded = urlParams.get('model');

    // modelUrl must be an url, need regexp check it
    if (modelUrlEnCoded && modelUrlEnCoded.match(/^(http|https):\/\//)) {
      setModelUrl(modelUrlEnCoded)
    }
  }, []);
  return (
              ...
              <div className="w-1/2 px-4 select-none">
                <div className="-ml-28 mr-28 rounded-3xl shadow-2xl p-8 flex flex-col space-y-4 bg-white ">
                  <div className="flex flex-row justify-end items-center">
                    <div className="-mt-20 p-4 h-32 w-32 border-8 border-black flex flex-col justify-center items-center text-5xl rounded-full shadow-2xl bg-white">
                  ...
                  </div>
                  {
                    shapeKeyValues.length > 0
                      ...
                      : <div className="flex flex-row justify-center items-center ">
                        <div className="animate-pulse text-4xl font-bold my-8 text-gray-300">LOADING</div>
                      </div>
                  }
              ...
              <Card >
                <CardBody>
                  ...
                  <h1 className="text-2xl font-bold flex flex-row items-center ">
                    <img alt="craft logo" src="/icon.svg" width={32} height={32} className="mr-2" /> Face Craft - Kig.Land</h1>
                  <p className="text-gray-400 mt-2">return back to <Link href="https://www.kig.land">www.kig.land</Link></p>
                  <Input
                    type="url"
                    className="w-80 mt-10"
                    label="🎭 Model URL"
                    placeholder="Please input kig.land model url" value={modelUrl} onChange={
                      (e) => {
